...
async def handle_video(websocket, video_processor, controller):
    """Handle WebSocket commands and send video frames."""
    try:
...
async def start_websocket_server(video_processor, controller):
    """Start WebSocket server for video."""
    async with websockets.serve(
        lambda ws: handle_video(ws, video_processor, controller), "0.0.0.0", 8767
    ):
        await asyncio.Future()

def run_websocket(video_processor, controller):
    """Run WebSocket server in a thread."""
    asyncio.run(start_websocket_server(video_processor, controller))
...
def main():
    ...
    websocket_thread = threading.Thread(target=run_websocket, args=(video_processor, controller))
